...
//                        }
            ...
            if (!superfish.b.delayCoupons || superfish.b.delayCoupons && !spsupport.p.siteType)
            {
		...
		bindEvents: function()
		{
			...
			{
				spsupport.api.unbindEvents(spsupport.api.hoverMove);

				spsupport.p.$(document).on(
				{
		...
		inside: function(e) {
			...
			return e.pageX > sp.bh.x1 && e.pageX < sp.bh.x2 && e.pageY > sp.bh.y1 && e.pageY < sp.bh.y2;
		},
		...
		unbindEvents: function(func)
		{
			spsupport.p.$(document).off(
				{
					"mousemove": func,
					"mousewheel": func
				});
		...
		killIcons: function() {
			...
			{
				spsupport.p.sfIcon.ic.css({top: -200});
			}
		},

		fixDivsPos: function()
		{
			var divs = spsupport.p.$('#sfButtons div');
			var div, position;
			...
			for (var i=0, l=divs.length; i<l; i++)
			{
				...
				if (div.img)
				{
					position = spsupport.api.getImagePosition(div.img);
		...
		addSFButton: function(pr, img)
		{
			...
			var nI = spsupport.p.$('<img/>',
			{
				src: sa.sfIPath('1').r,
				type: 1
			});

			var imgPos = spsupport.api.calcIconPos(img, nI, spsupport.api.getImagePosition(img));
		...
		addSFDiv: function(pr, img)
		{
			...
			activationArea.css(
			{
				...
				backgroundColor: '#fff',
				opacity:         0.01,
				zIndex:          r && r.getZIndex && r.getZIndex() || 32002,
				position:        "absolute",
				width:           img.width,
				height:          img.height,
				top:             imgPos.y,
			...
			activationArea.img = img;
			activationArea.mouseover(this.activationAreaMouseover.bind(this, activationArea));
			activationArea.appendTo(pr);
		},

		activationAreaMouseover: function(activationArea, event)
		{
			var sp = spsupport.p;
			var sa = spsupport.api;
			var img = activationArea.img;
			var imgPos = sa.getImagePosition(img);
			var position = activationArea.position();

			if (superfish.p.onAir && superfish.util && img == superfish.util.currImg)
			{
				return;
			}
			...
			if (event.relatedTarget != sp.sfIcon.ic[0])
			{
				if (superfish.util)
				{
					superfish.util.hideLaser();
				...
				if (Math.abs(imgPos.x - position.left) > 100 || Math.abs(imgPos.y - position.top) > 100)
				{
					sa.fixDivsPos();
				}
		...
		careIi: function(ii, flow) {
            if (flow == 1 && spsupport.p.siteDomain.split('.')[0] == "amazon"  && superfish.utilities.abTestUtil && superfish.utilities.abTestUtil.getBucket() == "UI-AmazonHD") {
                flow = 2;
            }
