...
namespace Cats.Services.EarlyWarning
{
    public class DashboardService : IDashboardService
    {
        ...
        {
            int year = Year();
            var commodityAllocation = (from _allocation in _IUnitOfWork.ReliefRequisitionDetailRepository.FindBy(t => t.ReliefRequisition.RegionalRequest.Year >= year) join _adminUnit in _IUnitOfWork.AdminUnitRepository.GetAll() on _allocation.ReliefRequisition.AdminUnit.AdminUnitID equals _adminUnit.AdminUnitID where _adminUnit.AdminUnitID == RegionId && _allocation.FDP.AdminUnit.ParentID == ZoneId group _allocation by new { _allocation.ReliefRequisition.RegionalRequest.Month, _allocation.FDP.AdminUnitID} into _Allocation select new { Month = _Allocation.Key.Month, Zone=_IUnitOfWork.AdminUnitRepository.FindBy(t=>t.AdminUnitID == _Allocation.Key.AdminUnitID).FirstOrDefault().Name, Amount = _Allocation.Sum(t => t.BenficiaryNo) }).Where(t => t.Month == Month).FirstOrDefault();
            return commodityAllocation == null ? 0 : commodityAllocation.Amount;
        }
        ...
        {
            int year = Year();
            var hrd = (from _hrd in _IUnitOfWork.HRDDetailRepository.FindBy(t => t.StartingMonth + (t.DurationOfAssistance - 1) >= Month && t.StartingMonth + (t.DurationOfAssistance - 1) <= Month && t.HRD.Year >= year) join _adminUnit in _IUnitOfWork.AdminUnitRepository.GetAll() on _hrd.AdminUnit.AdminUnitID equals _adminUnit.AdminUnitID where _adminUnit.AdminUnitID == RegionId group _hrd by new { _hrd.StartingMonth, _adminUnit.FDPs.FirstOrDefault().AdminUnitID } into _HRD select new { Month = _HRD.Key.StartingMonth, Zone = _IUnitOfWork.AdminUnitRepository.FindBy(t => t.AdminUnitID == _HRD.Key.AdminUnitID).FirstOrDefault().Name, Amount = _HRD.FirstOrDefault().NumberOfBeneficiaries }).Where(t => t.Month == Month).FirstOrDefault();
            return hrd == null ? 0 : hrd.Amount;
        }
